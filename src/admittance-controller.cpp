...
#define OUTPUT_SIGNALS      m_qDesSOUT << m_dqDesSOUT << FORCE_ERROR_SIGNALS
      ...
      DEFINE_SIGNAL_OUT_FUNCTION(qDes,dynamicgraph::Vector)
      {
        ...
        const dynamicgraph::Vector& dqDes  = m_dqDesSOUT(iter); // n
...
//                     "too far from q,  q-qDes="+toString(q(6+i)-m_qDes(i)), MSG_TYPE_WARNING);
      ...
      DEFINE_SIGNAL_OUT_FUNCTION(dqDes,dynamicgraph::Vector)
      {
        ...
        {
          const Eigen::Matrix<double, 6, 1> e_f_RF =      m_fRightFootErrorSOUT(iter); // 6
          const Eigen::Matrix<double, 6, 1> e_f_LF =      m_fLeftFootErrorSOUT(iter);  // 6
...
//          m_dqDes.head<6>() = svdSolveWithDamping(m_J_left_hand_svd,  e_f_LH, damping[3]);
      ...
      DEFINE_SIGNAL_OUT_FUNCTION(fRightFootError,dynamicgraph::Vector)
      {
        ...
        const Eigen::Matrix<double, 6, 1> f =           m_fRightFootSIN(iter);      // 6
        const Eigen::Matrix<double, 6, 1> fRef =        m_fRightFootRefSIN(iter);   // 6
      ...
      DEFINE_SIGNAL_OUT_FUNCTION(fLeftFootError,dynamicgraph::Vector)
      {
        ...
        const Eigen::Matrix<double,24,1> Kf =         m_KfSIN(iter); // 6*4
        const Eigen::Matrix<double, 6, 1> f =           m_fLeftFootSIN(iter);      // 6
        const Eigen::Matrix<double, 6, 1> fRef =        m_fLeftFootRefSIN(iter);   // 6
      ...
      DEFINE_SIGNAL_OUT_FUNCTION(fRightHandError,dynamicgraph::Vector)
      {
        ...
        const Eigen::Matrix<double,24,1> Kf =        m_KfSIN(iter); // 6*4
        const Eigen::Matrix<double, 6, 1> f =          m_fRightHandSIN(iter);      // 6
        const Eigen::Matrix<double, 6, 1> fRef =       m_fRightHandRefSIN(iter);   // 6
      ...
      DEFINE_SIGNAL_OUT_FUNCTION(fLeftHandError,dynamicgraph::Vector)
      {
        ...
        const Eigen::Matrix<double,24,1> Kf =         m_KfSIN(iter); // 6*4
        const Eigen::Matrix<double, 6, 1> f =           m_fLeftHandSIN(iter);      // 6
        const Eigen::Matrix<double, 6, 1> fRef =        m_fLeftHandRefSIN(iter);   // 6
