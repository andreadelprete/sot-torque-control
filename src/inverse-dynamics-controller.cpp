...
#define OUTPUT_SIGNALS m_tauDesSOUT << m_ddqDesSOUT << m_qErrorSOUT << \
      ...
      DEFINE_SIGNAL_OUT_FUNCTION(tauDes, dynamicgraph::Vector)
      {
        ...
        {
          const dynamicgraph::Vector& tauFB  = m_tauFBSOUT(iter); // n
          const dynamicgraph::Vector& tauFF  = m_tauFFSOUT(iter); // n
          const dynamicgraph::Vector& dynamicsError  = m_dynamicsErrorSIN(iter); // n+6
          const dynamicgraph::Vector& dynamicsErrorGain  = m_dynamicsErrorGainSIN(iter); // n+6
      ...
      DEFINE_SIGNAL_OUT_FUNCTION(ddqDes,dynamicgraph::Vector)
      {
        ...
        const Eigen::VectorXd Kp =m_KpSIN(iter); // n
        const Eigen::VectorXd Kd =m_KdSIN(iter); // n
        const Eigen::VectorXd q = m_base6d_encodersSIN(iter);     //n+6
        const Eigen::VectorXd dq =m_jointsVelocitiesSIN(iter);     // n
        const Eigen::VectorXd qRef = m_qRefSIN(iter);   // n
        const Eigen::VectorXd dqRef =     m_dqRefSIN(iter);  // n
        const Eigen::VectorXd ddqRef =    m_ddqRefSIN(iter); // n
      ...
      DEFINE_SIGNAL_OUT_FUNCTION(tauFF,dynamicgraph::Vector)
      {
        ...
        const Eigen::VectorXd q = m_base6d_encodersSIN(iter);    // n+6
        const Eigen::VectorXd dq =m_jointsVelocitiesSIN(iter);   // n
        const Eigen::VectorXd w_b =  m_baseAngularVelocitySIN(iter);   // 3
        const Eigen::VectorXd dw_b = m_baseAccelerationSIN(iter);   // 6
        const Eigen::VectorXd ddqRef =    m_ddqRefSIN(iter);             // n
      ...
      DEFINE_SIGNAL_OUT_FUNCTION(tauFB, VectorN)
      {
        if(!m_initSucceeded)
        {
        ...
        const Eigen::VectorXd Kp =m_KpSIN(iter); // n
        const Eigen::VectorXd Kd =m_KdSIN(iter); // n
        const Eigen::VectorXd Kf =m_KfSIN(iter); // 6*4
        const Eigen::VectorXd Ki =m_KiSIN(iter); // 6*4
        const Eigen::VectorXd qRef = m_qRefSIN(iter);   // n
        const Eigen::VectorXd dqRef =     m_dqRefSIN(iter);  // n
        ...
        if(m_fRightFootRefSIN.isPlugged())
        {
          ...
          const Eigen::VectorXd fRef, m_fRightFootRefSIN(iter);  // 6
          const Eigen::VectorXd f =  m_fRightFootSIN(iter);     // 6
...
//          SEND_DEBUG_STREAM_MSG("Force err RF integral: "+toString(m_f_RF_integral.transpose()));
        ...
        if(m_fLeftFootRefSIN.isPlugged())
        {
          const Eigen::VectorXd fRef, m_fLeftFootRefSIN(iter);  // 6
          const Eigen::VectorXd f =  m_fLeftFootSIN(iter);     // 6
        ...
        if(m_fRightHandRefSIN.isPlugged())
        {
          const Eigen::VectorXd fRef, m_fRightHandRefSIN(iter);  // 6
          const Eigen::VectorXd f =  m_fRightHandSIN(iter);     // 6
        ...
        if(m_fLeftHandRefSIN.isPlugged())
        {
          const Eigen::VectorXd fRef, m_fLeftHandRefSIN(iter);  // 6
          const Eigen::VectorXd f =  m_fLeftHandSIN(iter);     // 6
      ...
      DEFINE_SIGNAL_OUT_FUNCTION(tauFB2,VectorN)
      {
        if(!m_initSucceeded)
        {
        ...
        if(m_fLeftFootRefSIN.isPlugged())
        {
          const Eigen::VectorXd fRef, m_fLeftFootRefSIN(iter);
          const Eigen::VectorXd f =  m_fLeftFootSIN(iter);
        ...
        if(m_fRightHandRefSIN.isPlugged())
        {
          const Eigen::VectorXd fRef, m_fRightHandRefSIN(iter);
          const Eigen::VectorXd f =  m_fRightHandSIN(iter);
        ...
        if(m_fLeftHandRefSIN.isPlugged())
        {
          const Eigen::VectorXd fRef, m_fLeftHandRefSIN(iter);
          const Eigen::VectorXd f =  m_fLeftHandSIN(iter);
